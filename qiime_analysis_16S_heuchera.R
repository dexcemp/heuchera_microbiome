...
#Faith PD vs. predictors
temp <- ggplot(combined.reduced, aes(factor(BIOCLIM_1), faith_pd)) + stat_boxplot() + coord_flip() + xlab("") + ylab("Faith's PD") + ggtitle("Bacterial Faith's PD vs. temperature")
temp
...
tree <- read.tree("NuclearTreePartitionedByGene.rooted.FINAL16s.tre")
